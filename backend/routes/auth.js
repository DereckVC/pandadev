...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!email || !password || !passwordRegex.test(password)) return res.status(400).json({ message: 'La contraseña debe tener 8 caracteres, una mayúscula, una minúscula y un número.' });
    if (CANONICAL_ADMIN_EMAILS.includes(email.toLowerCase())) return res.status(403).json({ message: 'Este correo está reservado exclusivamente para inicio de sesión federado oficial (OAuth).' });
  ...
  } catch (error) { return res.status(400).json({ message: 'Unable to create account' }); }
});

router.post('/login', async (req, res) => {
  const user = await User.findOne({ email: req.body.email?.toLowerCase() }).select('+password +twoFactorSecret');
  if (!user || !user.password || !(await bcrypt.compare(req.body.password || '', user.password))) return res.status(401).json({ message: 'Invalid email or password' });
  if (user.provider === 'local' && !user.isVerified) return res.status(403).json({ message: 'Verifica tu correo antes de iniciar sesión.' });
  if (user.twoFactorEnabled) return res.json({ success: true, require2FA: true, userId: user._id, email: user.email });
  const deviceId = req.body.deviceId || req.header('x-device-id');
  const trustedToken = req.cookies?.trusted_device;
  if (isTrustedDevice(user.email, deviceId, trustedToken)) return sendSession(res, user);
  const otpCode = createOtp();
...
router.post('/verify-otp', async (req, res) => {
  ...
  if (!user || !user.otpCode || user.otpCode !== String(req.body.otp || '').trim() || !user.otpExpires || user.otpExpires <= new Date()) return res.status(400).json({ message: 'El código es incorrecto o ha expirado.' });
...
router.post('/2fa/enable', requireAuth, async (req, res) => {
  ...
  if (!base32 || !speakeasy.totp.verify({ secret: base32, encoding: 'base32', token: String(token || '').trim(), window: 1 })) return res.status(400).json({ message: 'El código de Google Authenticator no es válido.' });
...
router.post('/2fa/disable', requireAuth, async (req, res) => {
  ...
  if (!user.twoFactorEnabled || !user.twoFactorSecret || !speakeasy.totp.verify({ secret: user.twoFactorSecret, encoding: 'base32', token: String(req.body.token || '').trim(), window: 1 })) return res.status(400).json({ message: 'El código de Google Authenticator no es válido.' });
...
router.post('/reset-password/:token', async (req, res) => {
  if (!req.body.password || !passwordRegex.test(req.body.password)) return res.status(400).json({ message: 'La contraseña debe tener 8 caracteres, una mayúscula, una minúscula y un número.' });
...
const upsertOAuthUser = async (profile, provider) => {
  ...
  if (existing) {
    existing.provider = provider; existing.isVerified = true; existing.avatar = profile.photos?.[0]?.value || profile._json?.avatar_url || existing.avatar;
  ...
  return User.create({ email, name: profile.displayName || githubUsername, githubUsername, avatar: profile.photos?.[0]?.value || profile._json?.avatar_url || '', provider, role: isAdmin ? 'admin' : 'user', isVerified: true });
};
...
const updateProfile = async (req, res) => {
  ...
  if (req.body.currentPassword || req.body.newPassword) {
    ...
    if (!req.body.currentPassword || !req.body.newPassword || req.body.newPassword.length < 8) return res.status(400).json({ message: 'La contraseña actual y una nueva contraseña de 8 caracteres son obligatorias.' });
    const userWithPassword = await User.findById(req.user.id).select('+password');
    if (!userWithPassword.password || !(await bcrypt.compare(req.body.currentPassword, userWithPassword.password))) return res.status(401).json({ message: 'La contraseña actual no es correcta.' });
    userWithPassword.password = await bcrypt.hash(req.body.newPassword, 12);
    await userWithPassword.save();
  }
  req.user.name = name; req.user.discordTag = discordTag;
  await req.user.save(); return res.json({ user: publicUser(req.user) });
};
router.put('/profile', requireAuth, updateProfile);
router.patch('/profile', requireAuth, updateProfile);
router.put('/profile/avatar', requireAuth, async (req, res) => {
  if (typeof req.body.avatar !== 'string' || req.body.avatar.length > 2_000_000) return res.status(400).json({ message: 'La imagen no es válida o supera el límite permitido.' });
...
router.put('/change-password', requireAuth, async (req, res) => {
  if (req.user.provider !== 'local') return res.status(400).json({ message: 'Las cuentas OAuth gestionan la contraseña con su proveedor.' });
  if (!req.body.currentPassword || !passwordRegex.test(req.body.newPassword || '')) return res.status(400).json({ message: 'La nueva contraseña debe tener 8 caracteres, una mayúscula, una minúscula y un número.' });
  const user = await User.findById(req.user.id).select('+password');
  if (!user.password || !(await bcrypt.compare(req.body.currentPassword, user.password))) return res.status(401).json({ message: 'La contraseña actual no es correcta.' });
