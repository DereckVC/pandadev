...
const academicEmail = 'i2611843@continental.edu.pe'
...
function TechnologyCard({ title, description, items, muted = false }) {
  return <section className={`rounded-2xl bg-[#0d0d14]/90 border border-white/10 backdrop-blur-xl p-6 transition-all hover:border-[#8b5cf6]/40 ${muted ? 'opacity-90' : ''}`}>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
      {items.map(([name, icon]) => <div className="group flex flex-col items-center justify-center gap-2 min-h-[84px] rounded-xl border border-white/10 bg-white/[0.03] px-2 py-3 text-center transition-all hover:-translate-y-1 hover:border-[#8b5cf6]/50 hover:bg-[#8b5cf6]/10" key={name}>
...
function MetricCard({ icon, value, label, accent }) {
  return <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition-all hover:-translate-y-1 hover:border-[#8b5cf6]/50 hover:bg-white/[0.05]">
...
function LanguageBar({ name, percentage, color }) {
  return <div className="space-y-2">
    <div className="flex items-center justify-between text-sm">
      <span className="flex items-center gap-2 text-neutral-300"><i className="h-2 w-2 rounded-full" style={{ backgroundColor: color }} />{name}</span>
...
export default function About() {
  ...
  const copyEmail = async () => {
    if (!navigator.clipboard) {
      window.location.href = `mailto:${academicEmail}`
      return
    }
    await navigator.clipboard.writeText(academicEmail)
    setCopied(true)
    window.setTimeout(() => setCopied(false), 2200)
  }

  return <main className="w-full max-w-6xl mx-auto px-6 lg:px-12 py-10">
    ...
    <nav className="my-8 flex flex-wrap items-center gap-4" aria-label="Contacto rápido">
      ...
      <a className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-medium text-white transition-all hover:-translate-y-0.5 hover:border-[#8b5cf6] hover:bg-white/10" href="https://github.com/DereckVC" target="_blank" rel="noreferrer">
        <GithubIcon size={18} />
        {labels.github}
        <ArrowUpRight size={16} />
      </a>
      <a className="flex items-center gap-2.5 rounded-xl border border-[#5865F2]/40 bg-[#5865F2]/15 px-5 py-2.5 text-sm font-medium text-[#c7cbff] transition-all hover:-translate-y-0.5 hover:border-[#5865F2] hover:bg-[#5865F2]/25" href="https://discord.com" target="_blank" rel="noreferrer">
        <DiscordIcon size={18} />
        {labels.discord}
        <ArrowUpRight size={16} />
      </a>
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <MetricCard icon={<Code2 size={19} />} value="10+" label={labels.projects} accent="text-[#06b6d4]" />
      </div>
      <div className="rounded-2xl bg-[#0d0d14]/90 border border-white/10 p-6 backdrop-blur-xl">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {languages.map(([name, percentage, color]) => <LanguageBar key={name} name={name} percentage={percentage} color={color} />)}
        </div>
      </div>
      <div className="flex flex-col gap-2 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-5 py-3 text-sm text-emerald-300 sm:flex-row sm:items-center sm:justify-between">
        <strong className="flex items-center gap-2"><span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400 shadow-[0_0_12px_rgba(74,222,128,0.9)]" />{labels.status}</strong>
