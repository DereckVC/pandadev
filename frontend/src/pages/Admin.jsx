...
export default function Admin() {
  ...
  const markRead = async (id) => {
    try {
      await request(`/contact/${id}/read`, { method: 'PUT' })
      await loadMessages()
    } catch (requestError) {
      setError(requestError.message)
    }
  ...
  return (
    <main className="min-h-screen px-6 py-10 lg:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-[#0d0d14]/80 p-2">
          {tabs.map(([id, label]) => (
            <button className={`rounded-xl px-4 py-3 text-sm transition ${activeTab === id ? 'bg-[#8b5cf6] text-white' : 'text-neutral-400 hover:bg-white/10 hover:text-white'}`} type="button" key={id} onClick={() => setActiveTab(id)}>
              {label}
            </button>
          ))}
        </div>
        ...
        {notice && (
          <div className="mb-5 flex items-center gap-2 rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-3 text-sm text-emerald-200">
            <Check size={16} />{notice}
            <button className="ml-auto" type="button" onClick={() => setNotice('')}><X size={15} /></button>
          </div>
        )}
        ...
        {activeTab === 'messages' && (
          <section className="space-y-4">
            {messages.length === 0 ? <EmptyState text="No hay mensajes pendientes" /> : messages.map((item) => (
              <article className="rounded-2xl border border-white/10 bg-[#0d0d14]/90 p-5" key={item._id}>
                ...
                <p className="mt-4 rounded-xl bg-black/30 p-4 text-sm leading-relaxed text-neutral-300">{item.message}</p>
                <div className="mt-4 flex gap-2">
                  {!item.read && <button className="button button-outline" type="button" onClick={() => markRead(item._id)}>Marcar como leído</button>}
                  <button className="button border border-red-400/30 text-red-300 hover:bg-red-400/10" type="button" onClick={() => deleteMessage(item._id)}><Trash2 size={15} />Eliminar</button>
                </div>
              </article>
            ))}
          </section>
        ...
        {activeTab === 'projects' && (
          <section>
            <div className="mb-5 flex flex-wrap gap-3">
              <button className="button button-primary" type="button" onClick={() => openProject()}><Plus size={16} />Añadir Nuevo Proyecto</button>
              <button className="button button-outline" type="button" onClick={syncGithub} disabled={syncingGithub}>
                {syncingGithub ? <Activity className="animate-spin" size={16} /> : <Code2 size={16} />}
                {syncingGithub ? 'Sincronizando...' : 'Sincronizar con GitHub'}
              </button>
            </div>
            <div className="grid gap-4">
              {projects.map((project) => (
                <article className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-[#0d0d14]/90 p-5 md:flex-row md:items-center" key={project._id}>
                  <div className="flex-1">
                    <h2 className="font-bold text-white">{project.title}</h2>
                    <p className="mt-1 text-sm text-neutral-400">{project.category} · {(project.tags || []).join(', ')}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button className={`rounded-full px-3 py-1 text-xs transition ${project.showDemoBtn ? 'bg-purple-400/15 text-purple-300' : 'bg-white/10 text-neutral-400'}`} type="button" onClick={() => toggleDemo(project)}>
                      Demo {project.showDemoBtn ? 'On' : 'Off'}
                    </button>
                    <button className={`rounded-full px-3 py-1 text-xs transition ${project.showGithubBtn ? 'bg-emerald-400/15 text-emerald-300' : 'bg-white/10 text-neutral-400'}`} type="button" onClick={() => toggleGithub(project)}>
                      GitHub {project.showGithubBtn ? 'On' : 'Off'}
                    </button>
                  </div>
                  <div className="flex gap-2">
                    <button className="button button-outline" type="button" onClick={() => openProject(project)}><Pencil size={15} />Editar</button>
                    <button className="button border border-red-400/30 text-red-300" type="button" onClick={() => deleteProject(project._id)}><Trash2 size={15} /></button>
                  </div>
                </article>
              ))}
            </div>
        ...
        {activeTab === 'users' && (
          <section className="overflow-x-auto rounded-2xl border border-white/10 bg-[#0d0d14]/90">
            <table className="w-full min-w-[700px] text-left text-sm">
              ...
              <tbody>
                {users.map((account) => (
                  <tr className="border-b border-white/5 text-neutral-300" key={account._id}>
                    <td className="p-4 font-medium text-white">{account.name || 'Sin nombre'}</td>
                    <td className="p-4">{account.email}</td>
                    <td className="p-4"><span className="rounded-full bg-[#8b5cf6]/15 px-2 py-1 text-xs text-[#d8b4fe]">{account.role.toUpperCase()}</span></td>
                    <td className="p-4">{account.isVerified ? 'Verificado' : 'Pendiente'}</td>
                    <td className="p-4">{new Date(account.createdAt).toLocaleDateString()}</td>
                    <td className="p-4"><button className="text-[#c4b5fd] hover:text-white" type="button" onClick={() => toggleRole(account)}>Cambiar rol</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <form className="w-full max-w-2xl space-y-4 rounded-3xl border border-[#8b5cf6]/40 bg-[#0d0d14] p-6 shadow-2xl" onSubmit={saveProject}>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                ...
                ['slug', 'Slug'],
                ['category', 'Categoría'],
                ['bannerUrl', 'Imagen / Banner'],
                ['demoUrl', 'Demo URL'],
                ['repoUrl', 'GitHub URL']
              ].map(([key, label]) => (
                <label className="text-sm text-neutral-300" key={key}>
                  {label}
                  <input
                    className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-[#8b5cf6]"
            ...
            <label className="block text-sm text-neutral-300">
              ...
              <textarea
                ...
                onChange={(event) => setProjectForm({ ...projectForm, shortDesc: event.target.value })}
                required
              />
            </label>
            <label className="block text-sm text-neutral-300">
            ...
            <div className="flex flex-wrap gap-5 text-sm text-neutral-300">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(event) => setProjectForm({ ...projectForm, showGithubBtn: event.target.checked })}
                />
                GitHub visible
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(event) => setProjectForm({ ...projectForm, showDemoBtn: event.target.checked })}
                />
                Demo visible
              </label>
            </div>
            <button className="button button-primary w-full py-3 font-semibold" type="submit">
              Guardar Proyecto
            </button>
          </form>
        </div>
